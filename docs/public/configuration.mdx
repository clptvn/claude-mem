...
|-------------------------------|---------------------------------|---------------------------------------|
| `CLAUDE_MEM_MODEL`            | `claude-haiku-4-5-20251001`     | Primary compression model for the Claude or Codex provider |
...
| Setting | Default | Description |
|---------|---------|-------------|
| `CLAUDE_MEM_CODEX_PATH` | _(auto-detect)_ | Optional path to the Codex CLI |
| `CLAUDE_MEM_CODEX_REASONING_EFFORT` | `low` | Codex reasoning effort for memory extraction |
| `CLAUDE_MEM_CODEX_TIMEOUT_MS` | `180000` | Maximum time for one non-interactive Codex request |

The installer configures `gpt-5.6-luna` for routine observations and session
summaries, and `gpt-5.6-terra` for complex observation batches. See
[Codex Provider](usage/codex-provider).
...
| `CLAUDE_MEM_CODEX_PATH`       | _(auto-detect)_                 | Path to Codex CLI |
